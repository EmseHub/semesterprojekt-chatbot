...
export function getProcessedWords(strText) {
    ...
    const objDiagnostic = {};
    let strProcessedText = strText;

    // Alles außer Buchstaben, Zahlen und Bindestrichen mit ' ' ersetzen
    strProcessedText = strProcessedText.replace(/[^a-z0-9äöüß]/gi, ' ')

    // Jeden Whitespace (Space/Tab/Newline) beliebiger Länge reduzieren auf ein Leerzeichen (' ')
    strProcessedText = strProcessedText.trim().replace(/\s+/g, ' ');

    // Array aus Wörtern
    let arrWords = strProcessedText.split(' ');
